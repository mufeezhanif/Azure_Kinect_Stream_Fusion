...
def concatenated_fusion(color_image: np.ndarray, ir_image:np.ndarray)-> np.ndarray:
    ir_image_resized = cv2.resize(ir_image, (color_image.shape[1], color_image.shape[0]))
    ir_image_resized = np.tile(ir_image_resized[:, :, np.newaxis], (1, 1, 3))
    concatenated_image = np.hstack((color_image, ir_image_resized))
...
while True:
    ...
    concatenatedImage = concatenated_fusion(color_image=color_image,ir_image=ir_image)
    with rgbContainer:
        spaceForRGB.image(concatenatedImage,caption="RGB Stream",channels= "RGB", use_column_width= True)
    with irCol:
        spaceForIR.image(ir_image,caption="IR Stream",channels= "RGB", use_column_width= True)
    with depthCol:
        spaceForDepth.image(color_image,caption="Concatenated Image",channels= "RGB", use_column_width= True)
    i += 1
